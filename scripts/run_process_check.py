...
def check_remote_processes():
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        sftp.put('scripts/check_processes.py', f'{project_path}/scripts/check_processes.py')
        sftp.close()
        
        stdin, stdout, stderr = ssh.exec_command(f"python3 {project_path}/scripts/check_processes.py")
